    ...
    return (
        <>
            <span ref={buttonRef} className="inline-flex min-w-0">
                <Button size="small" type="text" className={buttonClassName || "!h-8 !max-w-[180px] !justify-start !rounded-full !px-2.5"} style={{ background: theme.node.fill, color: theme.node.text }} icon={<Settings2 className="size-3.5" />} aria-label={t("settings.image.title")} onClick={() => updateOpen(!open)}>
                    <span className="truncate">
                        {t("settings.image.summary", { quality: imageQualityLabel(quality), size: imageSizeLabel(activeSize), count })}
                    </span>
                </Button>
            </span>
            {panel}
