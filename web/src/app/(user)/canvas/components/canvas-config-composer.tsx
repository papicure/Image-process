        ...
        >
            <div className="mb-2 flex items-center justify-between gap-2">
                <div className="flex min-w-0 items-baseline gap-2">
                    <div className="shrink-0 text-xs font-semibold">{t("canvas.config.composePrompt")}</div>
                    <div className="truncate text-[11px] opacity-55">{t("canvas.composer.hint")}</div>
                </div>
                <Button size="small" type="text" className="!h-7 !w-7 !min-w-7 !p-0" icon={<X className="size-3.5" />} aria-label={t("common.close")} onClick={onClose} />
            </div>
...
function resourceLabel(input: NodeGenerationInput, inputs: NodeGenerationInput[]) {
    ...
    if (input.type === "image") return tr("seedance.reference.image", { index: index + 1 });
    if (input.type === "video") return tr("seedance.reference.video", { index: index + 1 });
    if (input.type === "audio") return tr("seedance.reference.audio", { index: index + 1 });
