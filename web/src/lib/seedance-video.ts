...
export const seedanceRatioOptions = [
    { value: "16:9", labelKey: "settings.video.size.landscape" },
    { value: "9:16", labelKey: "settings.video.size.portrait" },
    { value: "1:1", labelKey: "settings.video.size.square" },
    { value: "4:3", labelKey: "settings.video.size.standardLandscape" },
    { value: "3:4", labelKey: "settings.video.size.standardPortrait" },
    { value: "21:9", labelKey: "settings.video.size.cinema" },
    { value: "adaptive", labelKey: "settings.video.adaptive" },
] as const;
...
export function seedancePixelLabel(resolution: string, ratio: string) {
    ...
    if (normalizedRatio === "adaptive") return tr("settings.video.autoMatch");
    return seedancePixels[normalizedResolution][normalizedRatio] || "";
}

export function seedanceRatioLabel(value: string) {
    return tr(seedanceRatioOptions.find((item) => item.value === value)?.labelKey || "settings.video.adaptive");
}
...
export function seedanceReferenceLabel(kind: "image" | "video" | "audio", index: number) {
    if (kind === "image") return tr("seedance.reference.image", { index: index + 1 });
    if (kind === "video") return tr("seedance.reference.video", { index: index + 1 });
    return tr("seedance.reference.audio", { index: index + 1 });
}
...
export function buildSeedancePromptText(prompt: string, images: ReferenceImage[], videos: ReferenceVideo[], audios: ReferenceAudio[]) {
    ...
    return `${tr("seedance.reference.promptPrefix", { labels: labels.join(tr("seedance.reference.separator")) })}\n\n${text}`;
}
...
export function seedanceVideoReferenceError(videos: ReferenceVideo[]) {
    ...
    for (let index = 0; index < videos.length; index += 1) {
        ...
        if (video.bytes && video.bytes > SEEDANCE_REFERENCE_LIMITS.videoMaxBytes) return tr("seedance.reference.videoTooLarge", { label });
        if (video.durationMs) {
            if (video.durationMs < 2000 || video.durationMs > 15000) return tr("seedance.reference.videoDuration", { label });
            totalDurationMs += video.durationMs;
        }
        if (video.width && video.height) {
            if (video.width < 300 || video.width > 6000 || video.height < 300 || video.height > 6000) return tr("seedance.reference.videoDimensions", { label });
            const ratio = video.width / video.height;
            if (ratio < 0.4 || ratio > 2.5) return tr("seedance.reference.videoRatio", { label });
            const pixels = video.width * video.height;
            if (pixels < 640 * 640 || pixels > 2206 * 946) return tr("seedance.reference.videoPixels", { label });
        }
    }
    if (totalDurationMs > 15000) return tr("api.video.seedanceVideoTotalDuration");
    return "";
}

export function seedanceVideoReferenceHint() {
    return tr("seedance.reference.videoHint");
}
