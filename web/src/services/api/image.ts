...
function parseImageRatio(value: string) {
    ...
    if (parts.length !== 2) throw new Error(tr("api.image.invalidSizeFormat"));
    const w = Number(parts[0]);
    const h = Number(parts[1]);
    if (!Number.isFinite(w) || !Number.isFinite(h) || w <= 0 || h <= 0) throw new Error(tr("api.image.positiveRatio"));
    if (Math.max(w, h) / Math.min(w, h) > IMAGE_MAX_RATIO) throw new Error(tr("api.image.ratioTooWide"));
...
function validateImageSize(width: number, height: number) {
    if (!Number.isInteger(width) || !Number.isInteger(height) || width <= 0 || height <= 0) throw new Error(tr("api.image.positiveIntegerSize"));
    if (width % IMAGE_SIZE_STEP !== 0 || height % IMAGE_SIZE_STEP !== 0) throw new Error(tr("api.image.sizeStep"));
    if (Math.max(width, height) > IMAGE_MAX_EDGE) throw new Error(tr("api.image.edgeTooLarge"));
    if (Math.max(width, height) / Math.min(width, height) > IMAGE_MAX_RATIO) throw new Error(tr("api.image.ratioTooWide"));
    const pixels = width * height;
    if (pixels < IMAGE_MIN_PIXELS || pixels > IMAGE_MAX_PIXELS) throw new Error(tr("api.image.pixelRange"));
}
...
function resolveRequestSize(quality: string | undefined, size: string) {
    ...
    throw new Error(tr("api.image.invalidSizeFormat"));
}
...
function parseImagePayload(payload: ImageApiResponse) {
    if (typeof payload.code === "number" && payload.code !== 0) {
        throw new Error(payload.msg || tr("api.common.requestFailed"));
    }
    const images =
        payload.data
    ...
    if (images.length === 0) {
        throw new Error(tr("api.image.noImage"));
    }
...
function validateResponsePayload(payload: ResponseApiPayload) {
    if (typeof payload.code === "number" && payload.code !== 0) throw new Error(payload.msg || tr("api.common.requestFailed"));
    if (payload.error?.message) throw new Error(payload.error.message);
}

function validateGeminiPayload(payload: GeminiPayload) {
    if (payload.error?.message) throw new Error(payload.error.message);
    if (payload.promptFeedback?.blockReason) throw new Error(tr("api.image.geminiRejected", { reason: payload.promptFeedback.blockReason }));
}
...
function parseGeminiImagePayload(payload: GeminiPayload) {
    ...
    const images =
        payload.candidates
            ...
            .filter((value): value is string => Boolean(value))
            .map((dataUrl) => ({ id: nanoid(), dataUrl })) || [];
    if (!images.length) throw new Error(tr("api.image.geminiNoImage"));
    return images;
}
...
export async function requestEdit(config: AiConfig, prompt: string, references: ReferenceImage[], mask?: ReferenceImage, options?: RequestOptions) {
    ...
    if (requestConfig.apiFormat === "gemini") {
        if (mask) throw new Error(tr("api.image.geminiMaskUnsupported"));
        try {
...
export async function requestImageQuestion(config: AiConfig, messages: AiTextMessage[], onDelta: (text: string) => void, options?: RequestOptions) {
    ...
    const noContent = tr("api.image.noTextContent");
    try {
        if (requestConfig.apiFormat === "gemini") {
            const answer = (await requestGeminiStreamingResponse(requestConfig, toGeminiBody(requestConfig, messages), onDelta, options)).content || noContent;
            if (answer === noContent) onDelta(answer);
            return answer;
        }
        const answer = (await requestStreamingResponse(requestConfig, {
            model: requestConfig.model,
            input: toResponseInput(withSystemMessage(requestConfig, messages)),
        }, onDelta, options)).content || noContent;
        if (answer === noContent) onDelta(answer);
        return answer;
    } catch (error) {
        throw new Error(readAxiosError(error, tr("api.common.requestFailed")));
...
export async function fetchImageModels(config: Pick<AiConfig, "baseUrl" | "apiKey" | "apiFormat">) {
    try {
        ...
        throw new Error(readAxiosError(error, tr("api.image.readModelsFailed")));
    }
}
