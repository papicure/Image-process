...
export async function testWebdavConnection(config: WebdavSyncConfig) {
    ...
    await throwWebdavError(response, tr("config.webdav.testFailed"));
}
...
export async function downloadWebdavFile(config: WebdavSyncConfig, path: string) {
    ...
    if (!response.ok) await throwWebdavError(response, tr("webdav.error.readSyncFileFailed"));
    const file = await withTimeout(response.blob(), tr("webdav.error.readSyncFileTimeout"));
...
export async function uploadWebdavFile(config: WebdavSyncConfig, path: string, file: Blob, contentType = "application/octet-stream") {
    if (!file.size) throw new Error(tr("webdav.error.emptyUpload"));
    await ensureWebdavDirectory(config);
    ...
    if (!response.ok) await throwWebdavError(response, tr("webdav.error.uploadSyncFileFailed"));
}
...
async function ensureWebdavDirectoryPath(config: WebdavSyncConfig, directory: string) {
    ...
    for (const part of parts) {
        ...
        await throwWebdavError(response, tr("webdav.error.createRemoteDirFailed"));
    }
...
async function webdavFetch(config: WebdavSyncConfig, path: string, init: RequestInit) {
    ...
    try {
        ...
        if (error instanceof Error && error.name === "AbortError") throw new Error(tr("webdav.error.requestTimeout"));
        if (error instanceof TypeError) throw new Error(tr("webdav.error.unreachable"));
        throw error;
    } finally {
...
function assertWebdavConfig(config: WebdavSyncConfig) {
    if (!config.url.trim()) throw new Error(tr("config.webdav.needUrl"));
}

async function throwWebdavError(response: Response, fallback: string): Promise<never> {
    const detail = await response.text().catch(() => "");
    if (response.status === 401 || response.status === 403) throw new Error(tr("webdav.error.authFailed"));
    if (response.status === 404) throw new Error(tr("webdav.error.pathMissing"));
